...
class BdkModule(reactContext: ReactApplicationContext) :
  ...
  fun initWallet(
    ...
    try {
      ...
      this.wallet = BdkWallet( descriptors.externalDescriptor, descriptors.externalDescriptor, networkObj)
      result.resolve(true)
    } catch (e: Exception) {
      return handleReject(result, BdkErrors.init_wallet_failed, Error(e))
    }
  }
