...
class BdkInterface {
  ...
  constructor() {
    this._bdk = NativeModules.BdkModule;
  }
  ...
  async generateMnemonic(
    args: GenerateMnemonicRequest = { length: 12 }
  ): Promise<Response> {
    try {
  ...
  async createExtendedKey(args: CreateExtendedKeyRequest): Promise<Response> {
    try {
      ...
      const keyInfo: string = await this._bdk.getExtendedKeyInfo(
        network,
        mnemonic,
        password
      );
  ...
  async createXprv(args: CreateExtendedKeyRequest): Promise<Response> {
    try {
      const { network, mnemonic, password } = args;
      const keyInfo: CreateExtendedKeyResponse =
        await this._bdk.getExtendedKeyInfo(network, mnemonic, password);
  ...
  async createDescriptor(args: CreateDescriptorRequest): Promise<Response> {
    try {
      ...
      if (useMnemonic) {
        ...
        xprv = await (
          await this.createXprv({ network, mnemonic, password })
        ).data;
      }
      ...
      if (type != 'MULTI') {
        ...
        descriptor = `sh(multi(${threshold}${xprv},${publicKeys?.join(
          ','
        )}${path}))`;
      }
  ...
  async getPendingTransactions(): Promise<Response> {
    try {
      const response: Array<PendingTransaction> =
        await this._bdk.getPendingTransactions();
      return success(response);
    } catch (e: any) {
      return failure(e);
    }
  ...
  async getConfirmedTransactions(): Promise<Response> {
    try {
      const response: Array<ConfirmedTransaction> =
        await this._bdk.getConfirmedTransactions();
      return success(response);
    } catch (e: any) {
      return failure(e);
    }
  ...
  async getTransactions(): Promise<Response> {
    try {
      const confirmed: Array<ConfirmedTransaction> =
        await this._bdk.getConfirmedTransactions();
      const pending: Array<PendingTransaction> =
        await this._bdk.getPendingTransactions();
